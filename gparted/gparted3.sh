...
mkdir -p ~/BUA/.dep 2>/dev/null && cd ~/BUA/.dep && wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O ~/BUA/.dep/dep.zip https://github.com/Egide-C/BUA2/raw/main/.dep/dep2.zip && yes "y" | unzip -oq ~/BUA/.dep/dep.zip && cd ~/
